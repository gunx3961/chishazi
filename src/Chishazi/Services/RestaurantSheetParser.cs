...
public sealed class RestaurantSheetParser(TagSheetParser tagParser)
{
    public RestaurantParseResult Parse(SpreadsheetSnapshot spreadsheet)
    {
        var definition = SpreadsheetDefinition.Restaurant;
        var worksheet = spreadsheet.FindWorksheet(definition.Name);

        if (worksheet is null)
        {
            return new RestaurantParseResult([], []);
        }
        ...
        for (var rowIndex = 1; rowIndex < worksheet.Values.Count; rowIndex++)
        {
            ...
            {
                errors.Add(UiText.Get("RestaurantRowNameRequired", sheetRowNumber));
                continue;
            }
            ...
            errors.AddRange(unknownTags.Select(tag =>
                UiText.Get("UnknownRestaurantTag", sheetRowNumber, tag)));
            ...
            restaurants.Add(new RestaurantItem(
                ...
                tags,
                GetText(row, headers, "location").Trim()));
        }

        var duplicateNames = restaurants
            .GroupBy(restaurant => restaurant.Name, StringComparer.OrdinalIgnoreCase)
            .Where(group => group.Count() > 1)
            .Select(group => group.Key)
            .Order(StringComparer.OrdinalIgnoreCase);

        errors.AddRange(duplicateNames.Select(name =>
            UiText.Get("DuplicateRestaurantName", name)));

        return new RestaurantParseResult(restaurants, errors);
    }
